...
function selectPlayersNotInvited(gameid){
  ...
  $.ajax({
    url: "http://35.233.15.71/gameonphp/selectAllPlayersNotInvited.php",
    type: "post",
    data:{gameid:gameid},
...
function checkPassedGames(uname){
  ...
  $.ajax({
    url: "http://35.233.15.71/gameonphp/selectGamesThatOver.php",
    type: "post",
    ...
    success: function(data){
      $.each(data,function(i,item){
        ...
        if(item.profile_picture!=null  && item.profile_picture!="")
        {
          ...
          y.setAttribute("src", "http://35.233.15.71/gameonphp/upload/"+item.profile_picture);
...
function   updateRanking(playerId){
  ...
  setTimeout(function(){   swal("You can't rank yourself!");},50);
    ...
    $.ajax({
      url: "http://35.233.15.71/gameonphp/update_rank_user.php",
      type: "post",
  ...
  function selectAllPlayersToRank(game_id){
    ...
    $.ajax({
      url: "http://35.233.15.71/gameonphp/show_players_list.php",
      type: "post",
  ...
  function selectAllNotiOfPlayer(uname){
    ...
    $.ajax({
      url: "http://35.233.15.71/gameonphp/selectAllNoti.php",
      type: "post",
      data:{uname: uname},
      success: function(data){
        if(data.length!=0)
  ...
  function updateGameNoti(noti_id,game_id){
    ...
    $.ajax({
      url: "http://35.233.15.71/gameonphp/update_noti.php",
      type: "post",
  ...
  function addGameNoti(g_id,uname,picked_Users){
    ...
    $.ajax({
      url: "http://35.233.15.71/gameonphp/add_noti.php",
      type: "post",
  ...
  function selectAlluserd(){
    pageIsLoading();
    $.ajax({
      url: "http://35.233.15.71/gameonphp/selectAllUsers.php",
      type: "get",
      success: function(data){
        ...
        $.each(data,function(i,item){
          ...
          $('#allUsers').append('<li><a href="#" class="ui-btn ui-shadow ui-corner-all ui-btn-icon-right ui-icon-myicon2 loadinfo" onclick="profileDeatils(\'' +item.user_id+ '\');"><img src="http://35.233.15.71/gameonphp/upload/'+item.profile_picture+'"><h1>'+item.first_name+' '+item.last_name+' </h1><p>'+item.city+'</p></a></li>').listview('refresh');
        });
        hideLoading();
      }, error: function(data){hideLoading();}
    });
  ...
  function calculateAllNotiOfPlayer(uname){
    ...
    $.ajax({
      url: "http://35.233.15.71/gameonphp/calculateNumNoti.php",
      type: "post",
      data:{uname: uname},
      success: function(data){
        if(data.length!=0)
